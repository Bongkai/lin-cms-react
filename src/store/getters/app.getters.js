...
function permissionShaking(stageConfig, auths, user) {
...
export const sideBarList = (state) => {
  ...
  const sideBar = deepGetSideBar(authStageConfig(state), sideBarLevel)
  return sideBar
}
...
export const getStageByRoute = (path) => {
  const result = Object.getOwnPropertySymbols(stageMap)
    .find(key => (stageMap[key].route === path))
