...
class UploadImgsDemo extends React.Component {
  ...
  render() {
    const { rules, rules2, initData, initData1, remoteFuncName, fit } = this.state
    return (
      <div className='lin-container animated-base'>
        <div className='lin-title'>图片上传演示</div>
        <div className='lin-wrap'>
          <Form className='upload-imgs-demo-form' {...formItemLayout} colon={false}>
            <Item label='普通示例'>
              <UploadImgs ref={ref=>this['uploadEle3']=ref}
                rules={rules} multiple
              />
              <div>
                <Button
                  onClick={()=>this.getValue('uploadEle3')}
                >获取当前图像数据</Button>
              </div>
            </Item>
            <Item label='禁用'>
              <UploadImgs ref={ref=>this['uploadEle4']=ref}
                multiple disabled
              />
              <div>
                <Button
                  onClick={()=>this.getValue('uploadEle4')}
                >获取当前图像数据</Button>
              </div>
            </Item>
            <Item label='动图检测示例'>
              <UploadImgs ref={ref=>this['uploadEle32']=ref}
                rules={rules2} multiple animatedCheck
              />
              <div>
                <Button
                  onClick={()=>this.getValue('uploadEle32')}
                >获取当前图像数据</Button>
              </div>
            </Item>
            <Item label='禁用+初始化'>
              <UploadImgs ref={ref=>this['uploadEle5']=ref}
                value={initData} rules={rules} multiple disabled animatedCheck
              />
              <div>
                <Button
                  onClick={()=>this.getValue('uploadEle5')}
                >获取当前图像数据</Button>
              </div>
            </Item>
            <Item label='带初始化, 限制4至7张'>
              <UploadImgs ref={ref=>this['uploadEle1']=ref}
                value={initData} rules={rules} multiple
                minNum={4} maxNum={7}
              />
              <div>
                <Button
                  onClick={()=>this.getValue('uploadEle1')}
                >获取当前图像数据</Button>
              </div>
            </Item>
            <Item label='初始化, 可多选, 不立即上传'>
              <UploadImgs ref={ref=>this['uploadEle2']=ref}
                value={initData} rules={rules} multiple autoUpload={false}
              />
              <div>
                <Button
                  onClick={()=>this.getValue('uploadEle2')}
                >获取当前图像数据</Button>
              </div>
            </Item>
            <Item label='禁用+初始化+不预览'>
              <UploadImgs ref={ref=>this['uploadEle7']=ref}
                value={initData} rules={rules} multiple disabled preview={false}
              />
              <div>
                <Button
                  onClick={()=>this.getValue('uploadEle7')}
                >获取当前图像数据</Button>
              </div>
            </Item>
            <Item label='排序+固定数量'>
              <UploadImgs ref={ref=>this['uploadEle8']=ref}
                rules={rules} multiple sortable minNum={3} maxNum={3}
              />
              <div>
                <Button
                  onClick={()=>this.getValue('uploadEle8')}
                >获取当前图像数据</Button>
              </div>
            </Item>
            <Item label='仅最大数量'>
              <UploadImgs
                rules={rules} multiple maxNum={3}
              />
            </Item>
            <Item label='定制宽高+排序'>
              <UploadImgs
                value={initData} rules={rules} sortable
                width={200} height={150}
              />
            </Item>
            <Item label='重新初始化'>
              <UploadImgs ref={ref=>this['uploadEle9']=ref}
                value={initData1} rules={rules} sortable
                width={200} height={150}
              />
              <div>
            ...
            <Item label='远程方法'>
              <Radio.Group
                ...
                onChange={(ev)=>this.changeState('remoteFuncName', ev.target.value)}
              >
              ...
              <UploadImgs ref={ref=>this['uploadEle13']=ref}
                rules={rules} multiple
                customRequest={this[remoteFuncName]}
              />
              <div>
                <Button
                  onClick={()=>this.getValue('uploadEle13')}
                >获取当前图像数据</Button>
              </div>
            </Item>
            ...
            <Item label='自定义校验函数'>
              <UploadImgs ref={ref=>this['uploadEle33']=ref}
                rules={rules} multiple
                beforeUpload={this.beforeFunc}
              />
              <div>
                <Button
                  onClick={()=>this.getValue('uploadEle33')}
                >获取当前图像数据</Button>
              </div>
            </Item>
          </Form>
